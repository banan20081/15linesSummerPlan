    ...
    public Picture blur(int size) {
		...
		for(int row =0; row< pixels.length; row++){
			for(int col =0; col< pixels[0].length; col++){
				...
				int startR = Math.max(0, row-step);
				int startC = Math.max(0, col-step);
				int endR = Math.min(row+step, pixels.length);
				int endC = Math.min(col+step, pixels[0].length);
				...
				resultPixels[row][col].setRed(sumRed/count); 
				resultPixels[row][col].setGreen(sumGreen/count);
				resultPixels[row][col].setBlue(sumBlue/count);
			}
		}
